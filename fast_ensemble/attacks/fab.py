...
class FAB(Attack):
    def __init__(self, model, mean, std, norm='Linf', eps=None, steps=100, n_restarts=1,
                 alpha_max=0.1, eta=1.05, beta=0.9, verbose=False, seed=0,
                 targeted=False, n_classes=10):
        super(FAB, self).__init__("FAB", model, mean, std)
        self.norm = norm
    ...
    def get_diff_logits_grads_batch(self, imgs, la):
        ...
        g2 = torch.zeros([y.shape[-1], *imgs.size()]).cuda()
    ...
    def attack_single_run(self, x, y=None, use_rand_start=False):
        ...
        x = x.detach().clone().float().cuda()
        # assert next(self.model.parameters()).device == x.device

        y_pred = self._get_predicted_label(x)
        if y is None:
            y = y_pred.detach().clone().long().cuda()
        else:
        ...
        adv = im2.clone()
        adv_c = x.clone()
        res2 = 1e10 * torch.ones([bs]).cuda()
        res_c = torch.zeros([x.shape[0]]).cuda()
        x1 = im2.clone()
        ...
        while counter_restarts < 1:
            if use_rand_start:
                if self.norm == 'Linf':
                    t = 2 * torch.rand(x1.shape).cuda() - 1
