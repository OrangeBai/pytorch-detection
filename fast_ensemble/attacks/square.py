...
class Square(Attack):
    def __init__(self, model, mean, std, norm='Linf', eps=None, n_queries=500, n_restarts=1,
                 p_init=.8, loss='margin', resc_schedule=True,
                 seed=0, verbose=False, targeted=False):
        super(Square, self).__init__("Square", model, mean, std)
        self.norm = norm
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                ...
                n_queries = torch.ones(x.shape[0]).cuda()
                ...
                for i_iter in range(self.n_queries):
                    ...
                    new_deltas = torch.zeros([c, h, w]).cuda()
