...
import numpy as np
from imagenet.image_net_utils import *
from imagenet.imagenet_validation import validate, validate_pgd
...
def parse_args():
    ...
    parser.add_argument('-c', '--config', default='', type=str, metavar='Path',
...
logger = initiate_logger(os.path.join(MODEL_PATH, 'ImageNet', configs.output_name), configs.evaluate)
print = logger.info
...
def main():
    ...
    if not os.path.isdir(os.path.join(MODEL_PATH, 'ImageNet', configs.output_name)):
        os.makedirs(os.path.join(MODEL_PATH, 'ImageNet', configs.output_name))
    ...
    if configs.resume:
        resume_path = os.path.join(MODEL_PATH, 'ImageNet', configs.resume)
    ...
    train_loader, val_loader = get_loader(configs, train_dir, val_dir)
    ...
    for epoch in range(configs.TRAIN.start_epoch, configs.TRAIN.epochs):
        ...
        }, True, os.path.join(MODEL_PATH, 'ImageNet', f'{configs.output_name}'),
            epoch + 1)
...
def train(train_loader, model, criterion, optimizer, epoch, lr_schedule, half=False):
    ...
    iter_items = enumerate(train_loader)
    for i in range(num_iters):
        try:
            _, (input, target) = next(iter_items)
        ...
        for j in range(configs.ADV.n_repeats):
            ...
            if random.random() > 0.8 and configs.TRAIN.noisy:
