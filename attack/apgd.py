...
class APGD(Attack):
    def __init__(self, model, mean, std, norm='Linf', eps=8 / 255, steps=100, restart=1,
                 seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False):
        super(APGD, self).__init__("APGD", model, mean, std)
        self.eps = eps
        self.steps = steps
        self.norm = norm
        self.n_restarts = restart
        self.seed = seed
        self.loss = loss
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.norm == 'Linf':
            ...
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0], 1, 1, 1]).cuda().detach() * t / (
        ...
        step_size = self.eps * torch.ones([x.shape[0], 1, 1, 1]).cuda().detach() * torch.Tensor([2.0]).cuda()\
            .detach().reshape([1, 1, 1, 1])
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (
                        ...
                        self.eps * torch.ones(x.shape).cuda().detach(),
