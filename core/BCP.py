...
def train_BCP(loader, model, opt, epsilon, kappa, epoch, log, verbose, args, u_list):
    ...
    for i, (X, y) in enumerate(loader):
        ...
        bcp_loss, another_loss, bcp_err, another_err, u_list = BCP_loss(net_bcp, epsilon1,
                                                                        Variable(X), Variable(y), u_list, args.sniter,
                                                                        args.opt_iter, bce=args.bce, linfty=args.linfty)
        print(bcp_loss)
        ...
        if verbose and (i == 0 or i == len(loader) - 1 or (i + 1) % verbose == 0):
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                ...
                rloss=robust_losses, rerrors=robust_errors, rloss1=another_robust_losses,
...
def evaluate_BCP(loader, model, epsilon, epoch, log, verbose, args, u_list):
    ...
    for i, (X, y) in enumerate(loader):
        ...
        bcp_loss, another_loss, bcp_err, another_err, u_list = BCP_loss(net_bcp, epsilon,
                                                                        Variable(X), Variable(y), u_list,
                                                                        args.test_sniter, args.test_opt_iter,
                                                                        show=args.print, linfty=args.linfty)
...
class net_BCP(nn.Module):
    ...
    def bcp_leakyrelu(self, layer, mu, r, ibp_mu, ibp_r, i):
        ibp_ub = ibp_mu + ibp_r
        ibp_lb = ibp_mu - ibp_r
        ibp_ub = self._leakyrelu(ibp_ub)
        ibp_lb = self._leakyrelu(ibp_lb)
        ibp_mu = (ibp_ub + ibp_lb) / 2
        ibp_r = (ibp_ub - ibp_lb) / 2
        ...
        ibp_ub1 = self._leakyrelu(mu + r.view(-1, *mu_size))
        ibp_lb1 = self._leakyrelu(mu - r.view(-1, *mu_size))
        ibp_ub = torch.min(ibp_ub, ibp_ub1)
        ibp_lb = torch.max(ibp_lb, ibp_lb1)
        ibp_mu = (ibp_ub + ibp_lb) / 2
        ibp_r = (ibp_ub - ibp_lb) / 2

        mu = self._leakyrelu(mu)
        r = r
        self.u_list[i] = 1
        return mu, r, ibp_mu, ibp_r
    ...
    def bcp_bacthnorm1d(self, layer, mu, r, ibp_mu, ibp_r, i):
        ...
        mu_after = self._batchnorm1d(mu, mean, var, scale, bias, bn_eps)
        ww = scale * torch.rsqrt(var.view(-1) + bn_eps)
...
def BCP_translation(mu_prev, r_prev, ibp_mu_prev, ibp_r_prev, W, opt_iter, show):
    ...
    while (iteration == 0 or (
        ...
        if show:
            ...
            print('out ratio: %.8f' % (
                    ((diag_zero * wi_wj1 < lb) + (diag_zero * wi_wj1 > ub)).sum().float() / (b * c * c * h)),
                  end=' -> ')
            print('%.8f' % (((new_wi_wj < lb) + (new_wi_wj > ub)).sum().float() / (b * c * c * h)), end=' / ')
            LMT_ip = (wi_wj_rep.view(-1, h) * wi_wj1.view(-1, h)).sum(-1).view(b, c, c)
            print('t_BCP/t_LMT: %.4f' % (
                    (diag_zero * inner_prod.view(b, c, c, 1)).sum() / (diag_zero * LMT_ip.view(b, c, c, 1)).sum()))
